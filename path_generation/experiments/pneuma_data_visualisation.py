...
def min_max_locs():
    ...
    min_long_start = min(car_entries, key=lambda x: x[3])[3]
    min_lat_start = min(car_entries, key=lambda x: x[4])[4]
    #  find maximum start long and lat
    max_long_start = max(car_entries, key=lambda x: x[3])[3]
    max_lat_start = max(car_entries, key=lambda x: x[4])[4]
    #  find minimum end long and lat
    min_long_end = min(car_entries, key=lambda x: x[5])[5]
    min_lat_end = min(car_entries, key=lambda x: x[6])[6]
    #  find maximum end long and lat
    max_long_end = max(car_entries, key=lambda x: x[5])[5]
    max_lat_end = max(car_entries, key=lambda x: x[6])[6]
    ...
    ax.scatter(min_start_pos[0], min_start_pos[1], c="b", label="Min Start Pos")
    ax.scatter(max_start_pos[0], max_start_pos[1], c="r", label="Max Start Pos")
    ax.scatter(min_end_pos[0], min_end_pos[1], c="g", label="Min End Pos")
    ax.scatter(max_end_pos[0], max_end_pos[1], c="y", label="Max End Pos")
    ax.legend()
    ax.ticklabel_format(useOffset=False)

    # axis_min = (23.734619, 37.976679)
    # axis_max = (23.738652, 37.981113)
...
def create_zone_sensing_grid():
    ...
    plt.scatter(base0[0], base0[1], c="b", marker="s")
    plt.scatter(base1[0], base1[1], c="b", marker="s")
    plt.scatter(base2[0], base2[1], c="b", marker="s")
    plt.scatter(base3[0], base3[1], c="b", marker="s")
    #  write grid to file
