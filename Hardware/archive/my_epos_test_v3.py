...
def follow_epos(positions, hover_height, step_distance):
    ...
    for t in range(0,len(positions)):
        ...
        for agentID in range(0,len(allcfs.crazyflies)):
            ...
            move[agentID] = difference/np.absolute(difference) # normalise to have a magnitude of 1

            # The above doesn't work if target_position = current_position (move should be 0, but will be 1)
            for i in range(0,3):
                if difference[i] == 0:
                    move[agentID][i] = 0
        ...
        while (np.any(in_position == 0)): # while there exists at least one drone that is out of position
            for agentID in range(0,len(allcfs.crazyflies)): # will only run the paths for as many drones as are available (if there are paths for more drones than are active, then these paths won't be used)
                ...
                """
                With the basic_test_2, in_position[agentID] should be [0,1]
                So it should only activate the 2nd if statement
                Currently it activates both

                Need to print out values of if conditions to see why it's activating

                Also doesn't seem to be moving the drones even when the if statements are activated, despite the update vectors seeming correct?
                Wait nvm the vectors are both negative, whereas it should just be the x vector that's non-zero, and it should be positive
                """

                """
                # move at constant speed towards target (x,y) coord
                #If the drone is not at its target x position
                if (move[agentID][0]*(current_position[agentID][0]) < move[agentID][0]*(target_x)): # this ineqaulity is wrong
                    pos = current_position[agentID] + np.array([move[agentID][0]*step_distance, 0, 0])
                    cf.cmdPosition(pos)
                    print(np.array([move[agentID][0]*step_distance, 0, 0]))
                else:
                    in_position[agentID][0] = 1

                #If the drone is not at its target y position
                if (move[agentID][0]*(current_position[agentID][1]) < move[agentID][0]*(target_y)): # this ineqaulity is wrong
                    pos = current_position[agentID] + np.array([0, move[agentID][1]*step_distance, 0])
                    cf.cmdPosition(pos)
                    print(np.array([0, move[agentID][1]*step_distance, 0]))
                else:
                    in_position[agentID][1] = 1

                print(in_position[agentID])
                print(np.absolute(current_position[agentID]) - np.absolute(target_position))
                timeHelper.sleep(0.1)
                """
